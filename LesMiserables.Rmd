...
plot.igraph(g_w, layout=layout_gw, vertex.label=NA, vertex.size=5, edge.width=E(g_w)$weight)
```
...
```{r}
snowball_p <- function (g, seeds, waves = 1, p = 1,
                        return_subgraph = TRUE) {
  n <- vcount(g)
  marked <- rep(-1L, n) # all unmarked
  marked[seeds] <- 0 # mark seed set
  sel_edges <- logical(ecount(g)) # no edges selected
  for (w in seq_len(waves)) {
    for (v in which(marked >= 0)) { # sample
      Ev <- incident(g, v); nnv <- length(Ev)
      if (nnv > 0) {
        s <- rbinom(nnv, 1, p) == 1
        if (any(s)) {
          se <- Ev[s]
          sel_edges[se] <- TRUE
          sv <- V(g)[.inc(se)]; sv <- sv[marked[sv] < 0]
          marked[sv] <- w # tag wave
        }
      }
    }
    if (all(marked >= 0)) break
  }
  if (return_subgraph)
    subgraph.edges(g, E(g)[sel_edges])
  else
    list(edges = sel_edges, waves = split(seq_along(marked), marked)[-1])
}
```

```{r}
snowball_m <- function (g, seeds, waves = 1, m = 1,
                        return_subgraph = TRUE) {
  n <- vcount(g)
  marked <- rep(-1L, n) # all unmarked
  marked[seeds] <- 0 # mark seed set
  ...
  for (w in seq_len(waves)) {
    for (v in which(marked >= 0)) { # sample
      Ev <- incident(g, v); nnv <- length(Ev)
      if (nnv > 0) {
        se <- if (nnv <= m) Ev else sample(Ev, m)
        sel_edges[se] <- TRUE
        sv <- V(g)[.inc(se)]; sv <- sv[marked[sv] < 0]
        marked[sv] <- w # tag wave
      }
...
```{r}
nseeds <- 3; S <- sample(V(g), nseeds)

g_snow2 <- snowball_m(g, seeds=S, m=5)
...
```{r}
g_snow1 <- snowball_p(g, p=.1, seeds=S)

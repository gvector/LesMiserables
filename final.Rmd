...
The second table shows the structure of the dataset **edges**, the core of our analysis, where are indicated and encoded all the interactions between the characters in the book.
...
To do so we create a new dataframe from the already existing dataframe **edges** and *group by* the columns "from" and "to"; then we count the number of occurencies of this interactions.
...
Moreover we'll transofrm into a weighted graph the **merged_edges** dataframe. The *weights* of each edge will be based on the number of interactions between the characters, represented in the dataframe by `n`. 
```{r}
...
```{r}
b <- betweenness(g_w)
hist(b, breaks = 40, main= "Histogram of Betweenness",
     xlab= "Betweenness")
```
...
```{r}
plot(s, b, ylab="Betweenness", xlab= "Strength", main= "Betweenness vs Strength")
```
...
with(dd, plot(log(d), log(fd), main="Degree Distribution", xlab="Degree", ylab="Frequency of each Degree"))
```
...
hist(membership(ceb), breaks = 30, xlim=c(0,50), xlab= "Clusters", main= "Cluster Memebership distribution")
```
...
### Degree subgraphs
```{r}
# modificare breacks, titoli, commento
par(mfrow = c(2, 3))
d_c1 <- degree(g_chp[[1]])
hist(d_c1)
d_c2 <- degree(g_chp[[2]])
...
```{r}
par(mfrow = c(2, 3))
plot(s_c1, b_c1, ylab="Betweenness", xlab= "Strength")
plot(s_c2, b_c2, ylab="Betweenness", xlab= "Strength")
plot(s_c3, b_c3, ylab="Betweenness", xlab= "Strength")
plot(s_c4, b_c4, ylab="Betweenness", xlab= "Strength")
plot(s_c5, b_c5, ylab="Betweenness", xlab= "Strength")
plot(s,b)
```
...
# Conclusions
In conclusions the aim of the project was to analyze the dataset obtained from Victor Hugo's masterpiece "Les Miserables" from the perspective of a Network representation.
In doing this Network Analysis we looked at descriptive characteristics such as *Degree*, *Strength*, *betwenness* and *Transitivity*. Morover, we then looked for the "Important Actor" in the Network, discovering as may expected, it matched the Main Character of the Book (JEAN VALJEAN).
After that we examined the Degree Distribution Analysis of the Network looking for the Model which better explain the structure of it.

We then performed the Clustering Analysis, computing different methods, to find out the best way to divide our network into Comunities.

Lastly, we decided to divide the Network not following th e Cluster Analysis suggestion, but using the insights of the dataset, such as the Chapter Label. From this last analysis, we found out how the differents descriptive characteristics changes between the different chapters and also how the characters are distributed throught the book's components. 
